      ...
      <div class="row">
        <form id="roles-form" @keyup.enter="registrar()">
          <div class="col-md-10 col-md-offset-1">
            ...
            <div v-else>
              <div class="col-md-6">
                <template v-for="(input, i) in entries.izq">
                  <rs-input
                    :key="i"
                    :name="input"
                    required="true"
              ...
              <div class="col-md-6">
                <div
                  :key="i"
                  class="form-group"
                  v-for="(input, i) in entries.der"
                >
                  <label for="special" class="control-label">
                    <span :class="'fa fa-' + input.icon"></span>
                    {{ input.label }}
                  </label>
                  <date-picker
                    :id="input.id"
                    required="required"
                    v-model="formData.rol[input.id]"
                    :config="{ format: 'HH:mm:ss', useCurrent: false }"
                  />
  ...
  data() {
    return {
      entries: {
        ...
        der: [
          {
            label: "Hora a comenzar la actividad:",
            id: "from_at",
            icon: "calendar",
          },
          {
            label: "Hora a finalizar la actividad:",
            id: "to_at",
            icon: "calendar",
          },
        ],
  ...
  methods: {
    registrar: function () {
      ...
      if (this.formData.cond === "create") {
        axios.post(this.formData.url, this.formData.rol).then((response) => {
          toastr.success("Rol Creado Exitosamente");
          this.$emit("input");
          $("#rol-form").modal("toggle");
        });
      } else {
        axios.put(this.formData.url, this.formData.rol).then((response) => {
