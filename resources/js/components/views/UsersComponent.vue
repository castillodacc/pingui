<template>
  <div class="box">
    ...
    <div class="box-body">
      <rs-table id="users" :tabla="tabla" uri="/admin/users" />
    </div>
  </div>
</template>
...
import rsModalForm from "./../forms/modal-form-user.vue";
  ...
  methods: {
    openform: function (cond, id = null) {
      ...
      if (cond == "create") {
        ...
        axios.get(this.formData.url).then((response) => {
          this.formData.ico = "edit";
          this.formData.title =
            "Editar Usuario: " + response.data.fullName + ".";
