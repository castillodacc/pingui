	...
	<div class="row">
		<div class="col-md-12">
			<form class="row" @submit.prevent="register" v-if="r == true">
				...
				<div class="col-md-6" style="height: 75px" v-show="form.category_id == 1">
					<div class="form-group">
...
<script>
	...
	export default {
		...
		computed: {
			pay: function () {
				let age = 0;
				if (! this.form.category_id) {return this.form.pay = 0}
				let cat = (this.form.category_id == 1) ? 1 : 2
				for (let a of this.form.age_group) {
					age += (a == 0) ? 3 : 5
				}
